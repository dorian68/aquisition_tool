...
class ExcelLayoutEngine:
    ...
    def _linked_button_png(
        ...
        *,
        ...
        """Render a clickable dashboard/navigation button as a PNG.

        This preserves the premium DA. Native Excel form buttons are intentionally
        kept for VBA actions only because they are visually hard to style.
        """
        ...
        draw.rounded_rectangle(
            ...
            outline=self._hex_to_rgba(line, 18),
            width=max(1, scale),
        )
        if accent:
            dot_r = max(3, min(5, rect.h // 5)) * scale
            cx = 15 * scale
            cy = rect.h * scale // 2
            draw.ellipse([cx - dot_r, cy - dot_r, cx + dot_r, cy + dot_r], fill=self._hex_to_rgba(accent, 0))

        try:
            font = ImageFont.truetype("DejaVuSans.ttf", 9 * scale)
        except Exception:
            font = None
        tx = 28 * scale if accent else 12 * scale
        ty = max(2, (rect.h * scale - 12 * scale) // 2)
        draw.text((tx, ty), str(text), fill=self._hex_to_rgba(text_color, 0), font=font)
    ...
    def image_link_button(
        ...
        *,
        ...
        png = self._linked_button_png(rect, text, fill=fill, line=line, text_color=text_color, accent=accent or self.theme.cyan)
        if png is not None:
    ...
    *,
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter", datetime_format="yyyy-mm-dd") as writer:
        ...
        for i, (item, target_sheet, target_cell) in enumerate(sidebar_items):
            y = 198 + i * 34
            active = item in {"Overview", "Graphs"}
